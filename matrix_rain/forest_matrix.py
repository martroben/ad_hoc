...
class Logo:
    ...
    def get_binary_form(self) -> list[list[bool]]:
        """
...
# Generated by https://seotoolbelt.co/tools/ascii-art-generator/#text-list-tab
with open("matrix_rain/ascii_logo.txt") as logo_file:
    logo_text = logo_file.read()

logo = Logo(logo_text)
logo_binary_scaled = logo.get_scaled_binary(Matrix.N_ROWS, Matrix.N_COLUMNS)

substitute_characters = [chr(character) for character in Matrix.CHARACTER_CODE_POINTS]

substituted_logo = ""
for row in logo_binary_scaled:
    substituted_row = ""
    for character in row:
        if character:
            substituted_row += random.choice(substitute_characters)
        else:
            substituted_row += Logo.BLANK_CHARACTER
    substituted_logo += substituted_row + Logo.NEWLINE_CHARACTER


print(substituted_logo)
